  ...
  return (
    <AnimatePresence>
      {enabled && hovered && hoverRect && (
        <motion.div
          key="sb001-hover-bg"
          className="pointer-events-none absolute z-0 rounded-md bg-muted/80"
          style={{ right: 0 }}
          initial={false}
          animate={{
            top: hoverRect.top + 2,
    ...
    <div className="relative">
      ...
      >
        <a
          ...
          className={cn(
            "relative flex items-center justify-between gap-2 ml-2 pl-4 py-1.5 text-xs select-none transition-colors",
            isActive
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            {icon && (
              <span
                className={cn(
                  "shrink-0 [&_svg]:size-4 transition-colors",
                  isActive ? "text-accent-pro" : "text-foreground/60"
                )}
              >
                {icon}
              </span>
            ...
            {count !== undefined && count !== null && (
              <span
                className={cn(
                  ...
                  isActive
                    ? "bg-accent-pro/15 text-accent-pro"
...
export function Sidebar001Separator({
  ...
  return (
    <div
      className={cn(
        "px-0 py-2.5 mt-1 text-xs font-bold uppercase tracking-wider text-foreground/50",
        className,
      )}
    >
      {children}
  ...
  return (
    <div className={cn("flex flex-col mb-1", className)}>
      <button
        ...
        className="relative z-1 flex items-center gap-2 py-1.5 px-2 select-none text-left w-full group rounded-md hover:bg-muted/40 transition-colors"
      >
        {icon ? (
          <>
            <span className="shrink-0 text-foreground/50 group-hover:text-foreground/80 [&_svg]:size-4">
              {icon}
            </span>
            <span className="text-xs font-semibold text-foreground/70 group-hover:text-foreground transition-colors duration-150 flex-1">
              {label}
            </span>
            <MotionChevron
              size={14}
              strokeWidth={2.5}
              className="shrink-0 text-foreground/40 mr-1"
          ...
          <>
            <MotionChevron
              ...
              transition={{ type: "spring", stiffness: 500, damping: 30 }}
            />
            <span className="text-xs font-semibold text-foreground/70 group-hover:text-foreground transition-colors duration-150">
              {label}
            </span>
          </>
        )}
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="flex flex-col pl-2 py-0.5 space-y-0.5">{children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
export function Sidebar001Content({
  ...
  return (
    <div
      className={cn("flex-1 overflow-y-auto py-2 custom-scrollbar", className)}
...
export interface Sidebar001Props {
  ...
  defaultWidth?: number;
  /** Min resize width in px. Default: 180 */
  minWidth?: number;
  /** Max resize width in px. Default: 420 */
  maxWidth?: number;
}
  ...
  return (
    <EffectsProvider defaultEnabled={defaultEffectsEnabled}>
      <HoverProvider containerRef={containerRef}>
        <aside
          className={cn(
            "relative flex flex-col h-full shrink-0 bg-background border-r border-border/60",
            className,
          )}
        ...
        >
          ...
          {/* Resize handle */}
          <div
            className="hidden lg:block absolute top-0 right-0 h-full w-1 cursor-col-resize group/handle z-20"
...
export function Sidebar001Header({
  ...
  return (
    <div className={cn("shrink-0 px-3 pt-3 pb-2 border-b border-border/40", className)}>{children}</div>
  );
}
...
export function Sidebar001Footer({
  ...
  return (
    <div
      className={cn(
        "shrink-0 px-3 pb-3 pt-2 border-t border-border/50",
        className,
      )}
    >
      {children}
