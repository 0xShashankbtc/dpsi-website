      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[280px] text-[12px] bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200/80 dark:border-slate-700/60">
              {active.meta.map((row) => (
                <div key={row.label} className="flex justify-between py-[5px] border-b border-slate-100 dark:border-slate-700/30 last:border-0">
          ...
          {slides.map((_, index) => (
            <button
              ...
              className={cn(
                ...
                index === selected ? "opacity-100 w-5 rounded-full" : "opacity-30",
              )}
            />
          ))}
        </div>
