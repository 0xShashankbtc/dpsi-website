        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            ...
            >
              ...
              <div className="absolute top-0 right-0 w-64 h-64 rounded-full bg-emerald-500/15 blur-3xl pointer-events-none" />
              <div className="absolute top-1/3 left-0 w-64 h-64 rounded-full bg-amber-500/10 blur-3xl pointer-events-none" />
              <div className="absolute bottom-0 right-0 w-64 h-64 rounded-full bg-emerald-600/15 blur-3xl pointer-events-none" />

              {/* STICKY TOP HEADER */}
              <div className="sticky top-0 z-40 shrink-0 p-3.5 bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-950 text-white flex items-center justify-between shadow-md border-b border-emerald-500/30">
                <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-600 to-amber-500 flex items-center justify-center text-white shadow-md shrink-0">
              ...
              <div data-lenis-prevent aria-live="polite" aria-atomic="false" className="flex-1 overflow-y-auto p-3.5 sm:p-4 space-y-3 relative z-10 custom-scrollbar overscroll-contain">
                  ...
                  >
                    <div
                      className={`max-w-[85%] rounded-2xl p-3.5 text-xs sm:text-sm leading-relaxed shadow-sm relative break-words [word-break:break-word] ${
                        msg.role === "user"
                          ? "bg-slate-900 dark:bg-emerald-900 text-white rounded-br-xs font-medium"
                          : "bg-white/95 dark:bg-slate-800/95 text-slate-800 dark:text-slate-100 rounded-bl-xs border border-white/80 dark:border-slate-700/80 shadow-md backdrop-blur-md"
                      }`}
                    >
                      ...
                      {msg.role === "assistant" && !msg.isStreaming && (
                        <div className="flex items-center gap-2 mt-2 pt-1.5 border-t border-slate-200/50 dark:border-slate-700/50">
                          <button
                            ...
                            aria-label={isSpeaking ? "Stop voice reading" : "Play voice answer"}
                ...
                {isTyping && (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="flex items-center gap-2 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md px-3.5 py-2.5 rounded-2xl rounded-bl-xs border border-emerald-500/20 dark:border-slate-700/80 shadow-md w-max"
                  >
                    <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-emerald-600 to-amber-500 flex items-center justify-center text-white shadow-sm animate-spin-slow">
                      <Bot className="w-3.5 h-3.5 text-white" />
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="w-1.5 h-4 bg-emerald-500 rounded-full animate-bounce [animation-delay:-0.3s]" />
                      <span className="w-1.5 h-4 bg-amber-500 rounded-full animate-bounce [animation-delay:-0.15s]" />
              ...
              <div className="px-3 pt-2 pb-1.5 border-t border-slate-200/50 dark:border-slate-800/50 bg-white/40 dark:bg-slate-900/40 backdrop-blur-md shrink-0">
                <div className="grid grid-cols-2 gap-1.5 py-0.5">
                  {[
                    { label: "Admissions 2026", icon: <GraduationCap className="w-3 h-3 text-emerald-700 dark:text-emerald-400" />, query: "Tell me about admissions 2026" },
                    { label: "Academic Calendar", icon: <Calendar className="w-3 h-3 text-amber-600 dark:text-amber-400" />, query: "What is the 2026-27 Academic Calendar schedule for exams, breaks, and PTMs?" },
                    { label: "AI Robotics Lab", icon: <Bot className="w-3 h-3 text-emerald-600 dark:text-emerald-300" />, query: "Tell me about your AI Robotics Lab" },
                    { label: "Class 11 Streams", icon: <MessageSquare className="w-3 h-3 text-amber-700 dark:text-amber-300" />, query: "What streams are offered in Class 11?" },
                  ].map((chip, i) => (
                    <button
                      key={i}
                      onClick={() => {
                      ...
                      disabled={isTyping}
                      className="w-full px-2 py-1.5 rounded-xl bg-white/90 dark:bg-slate-800/90 hover:bg-white dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 font-bold text-[11px] truncate transition-all flex items-center justify-center gap-1.5 shadow-2xs hover:border-emerald-500/40 disabled:opacity-50 cursor-pointer"
                    >
                      {chip.icon}
              ...
              >
                ...
                <div className="w-full flex items-center gap-1.5 p-1 rounded-full bg-white/10 border border-white/20 focus-within:ring-2 focus-within:ring-emerald-400 transition-all">
                  ...
                  <input
                    type="text"
                    id="dpsi-ai-chat-input"
                    aria-label="Ask DPSI AI a question"
                    value={input}
                  ...
                  <button
                    ...
                    aria-label="Send query to DPSI AI"
                    className="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-600 to-amber-500 text-white flex items-center justify-center transition-all disabled:opacity-40 shrink-0 shadow-sm cursor-pointer touch-manipulation"
                  >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-3 px-3.5 py-2 sm:px-4 sm:py-2.5 rounded-full bg-gradient-to-r from-white via-slate-50 to-emerald-50 dark:from-slate-900 dark:via-slate-950 dark:to-emerald-950 text-slate-900 dark:text-white font-bold text-xs shadow-2xl shadow-slate-900/30 border border-emerald-500/30 backdrop-blur-2xl cursor-pointer pointer-events-auto select-none transition-shadow hover:shadow-emerald-500/20 touch-manipulation"
              title="Click to open DPSI AI Assistant"
              aria-label="Click to open DPSI AI Assistant"
            >
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-600 to-amber-500 flex items-center justify-center text-white shadow-md shrink-0 pointer-events-none">
                <Bot className="w-4 h-4 text-white" />
              </div>
              <div className="text-left pr-1 pointer-events-none">
                <p className="font-extrabold text-slate-900 dark:text-white text-xs leading-none">{aiBotButtonText}</p>
                <p className="text-[10px] text-emerald-700 dark:text-emerald-400 font-semibold leading-none mt-1">Ask Anything</p>
              </div>
            </motion.button>
          )}
        </AnimatePresence>
